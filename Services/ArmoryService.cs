...
namespace LegendaryArmory.Services
{
    internal class ArmoryService : IDisposable
    {
        ...
        public List<LegendaryItem> legendaries = new();
        ...
        public void InitLegendaries(IGw2WebApiV2Client apiClient)
        {
            try
            {
                ...
                legendaryIds.Remove(legendaryIds.Find(item => item.Id == 95093));

                LegendaryItems = (List<Item>)apiClient.Items.ManyAsync(legendaryIds.Select(item => item.Id)).Result.ToList();
                ...
                {
                    legendaries.Add(ConvertToLegendaryItem(item));
                }

                //Save data to local storage
                IOHelper.SaveItemData(legendaries);
            }
            catch (Exception ex)
            {
                Logger.Warn(ex, "Failed to get Legendaries from API: " + ex.Message);
                //Load data from local storage
                legendaries = IOHelper.LoadItemData();
            }
        ...
        private LegendaryItem ConvertToLegendaryItem(Item item)
        {
            LegendaryItem legen;
            ...
            switch (item.Type.ToEnum())
            {
                case ItemType.Weapon:
                    legen = new LegendaryWeapon();
                    AddWeaponValues((ItemWeapon)item, (LegendaryWeapon)legen);
                    break;
                case ItemType.Armor:
                    legen = new LegendaryArmor();
                    AddArmorValues((ItemArmor)item, (LegendaryArmor)legen);
                    break;
                case ItemType.Trinket:
                    legen = new LegendaryTrinket();
                    AddTrinketValues(item, (LegendaryTrinket)legen);
                    break;
                case ItemType.UpgradeComponent:
                    legen = new LegendaryUpgrade();
                    AddUpgradeValues(item, (LegendaryUpgrade)legen);
                    break;
                default:
                    legen = new LegendaryItem();
                    break;
            }

            //Generic item values
            legen.Id = item.Id;
            legen.Name = item.Name;
            legen.Icon = item.Icon;
            //legen.Amount = 0;
            legen.MaxAmount = legendaryIds.Find(a => a.Id == item.Id).MaxCount;
            legen.Level = item.Level;
            legen.Description = item.Description;
            legen.Type = item.Type;

            foreach (var flag in item.Flags)
            {
                legen.Flags.Add(flag);
            }

            return legen;
        }

        private void AddWeaponValues(ItemWeapon item, LegendaryWeapon weapon)
        {
            weapon.Generation = item.Id switch
            {
                <= 30704 => 1,
                > 30704 and <= 90551 => 2,
                > 90551 and <= 97783 => 3,
            ...
            weapon.MinPower = item.Details.MinPower;
            weapon.MaxPower = item.Details.MaxPower;
            weapon.DamageType = item.Details.DamageType;
            weapon.WeaponType = item.Details.Type;
        }

        private void AddArmorValues(ItemArmor item, LegendaryArmor armor)
        {
            armor.Slot = item.Details.Type;
            armor.WeightClass = item.Details.WeightClass;
            armor.Defense = item.Details.Defense;
        }

        private void AddTrinketValues(Item item, LegendaryTrinket trinket)
        {
            trinket.TrinketType = ((ItemTrinket)item).Details.Type;
        }

        private void AddUpgradeValues(Item item, LegendaryUpgrade upgrade)
        {
            upgrade.UpgradeType = item.Id == 101582 ? "Relic" : ((ItemUpgradeComponent)item).Details.Type;
        }
